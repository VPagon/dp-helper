...
1. **CDC pipeline** (`rep_mda.mda_ocn_pipelines`) — `MS_SQL_CDC_INFOR_DBO_{FULL}_TO_BRONZE_LANDING_ZONE`, `pipeline_type='METADATA_DRIVEN_INGESTION'`, `load_category='standard_load'` (default), `pipeline_priority=90`.
2. **Snapshot pipeline** — `MS_SQL_INFOR_DBO_{FULL}_TO_BRONZE_ZONE`, same pipeline_type, `load_category='irregular_dq'` (default) — this pipeline was missing from the old generator entirely.
3. **RDL pipeline** — `RDL_R_LN_DBO_{FULL}_DELTA`, `pipeline_type='LOAD_RAW_DELTA'`, `load_category='standard_load'`. `execution_resource`/`orchestrator_tool` are intentionally omitted from the INSERT column list — both have DB defaults (`SPARK_JOB` / `Synapse Pipeline`).
...
5. **CDC ingestion job** (`rep_mda.mda_data_ingestion`) — `DatastoreType='RimacOnPremSQLServerTableCdc'`, `dataLoadingBehavior='DeltaLoad'`.
6. **Snapshot ingestion job** — `DatastoreType='RimacOnPremSQLServerTable'`, `dataLoadingBehavior='FullLoad'`. RDL pipelines never get an ingestion job (confirmed empirically — only CDC and snapshot do).
...
8. **Dependency: RDL → CDC** (`rep_mda.mda_ocn_pipeline_dependencies`) — RDL depends on its CDC landing pipeline, `additional_checks='CHECK_DEPENDENCY_ROWCOUNT'`.
...
Folder-path/copy-activity JSON fields (`sink_object_settings`, `copy_activity_settings`) use literal macro placeholders (`#schema#`, `#table#`, `#source#`, `#pipeline#`, etc.) resolved by the Synapse pipeline engine at runtime — the wizard does not substitute these. Only `source_object_settings` (schema/table) and `mda_rdl_tables` source/target directories use real substituted values.
...
- All string values are SQL-escaped (`'` → `''`); JSON blob fields are built via `JSON.stringify` rather than manual string concatenation to avoid quoting/escaping bugs.
