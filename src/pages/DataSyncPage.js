...
export default function DataSyncPage() {
    ...
    const [previewEnv, setPreviewEnv] = useState('source'); // 'source' or 'target'
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [success, setSuccess] = useState(null);
    const [syncProgress, setSyncProgress] = useState({});
    const [batchSize, setBatchSize] = useState(1000);
    const [insertBatchSize, setInsertBatchSize] = useState(100); // Number of rows per INSERT statement
    ...
    const fetchEnvironments = async () => {
        try {
            ...
            setError(`Failed to fetch environments: ${err.message}`);
        }
    };
    ...
    const fetchTableColumns = async (tableName) => {
        ...
        try {
            ...
            const result = await executeQuery(
                ...
                AND TABLE_SCHEMA = '${tableName.split('.')[0]}'
            ...
            while (offset < totalCount) {
                ...
                for (let i = 0; i < batchData.length; i += insertBatchSize) {
                    ...
                    if (insertBatch.length > 0) {
                        const columns = sourceResult.columns.join(', ');
                        const valuesBatches = [];
