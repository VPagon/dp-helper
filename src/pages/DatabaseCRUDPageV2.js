    ...
    const generateUpdateSQL = () => {
        ...
        results.columns.forEach((column, index) => {
            if (column !== primaryKeyColumn && editData[column] !== selectedRow[index]) {
                ...
                let value;
                if (editData[column] === null) {
                    value = 'NULL';
                } else {
                    // Convert to string, trim, and escape single quotes
                    const stringValue = String(editData[column]).trim();
    ...
    const generateInsertSQL = () => {
        ...
        Object.entries(editData).forEach(([column, value]) => {
            ...
            if (value !== null && value !== undefined && value !== '') {
                // Convert value to string and trim, but handle non-string values
